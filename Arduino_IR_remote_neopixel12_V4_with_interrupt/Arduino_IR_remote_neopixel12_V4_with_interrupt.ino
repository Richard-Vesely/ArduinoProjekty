...
4) Proč porovnáváme IR kódy výhradně s konstantami z myIRcodes.h (a ne „0x…“)?
...
unsigned long fastStepDelayMs = 15;          // rychlejší animace když IR je neaktivní
unsigned long slowStepDelayMs = 30;          // normální rychlost když IR je aktivní

// LED update optimization - dynamic based on IR activity
unsigned long lastLedUpdateMs = 0;
unsigned long ledUpdateIntervalMs = 200;      // Dynamic LED update interval (ms)
bool ledUpdatePending = false;               // Flag to track if LEDs need updating

// IR activity tracking for dynamic timing
unsigned long lastIRSignalMs = 0;            // Last time an IR signal was received
unsigned long irActivityTimeoutMs = 750;    // Consider IR active for 1 second after last signal
unsigned long minLedIntervalMs = 10;        // Fastest LED updates (5 FPS) when IR is idle
unsigned long maxLedIntervalMs = 600;        // Slowest LED updates (2.5 FPS) when IR is active
...
void clampSpeed() {
  // Clamp the base speeds, but don't override the dynamic IR-based speeds
  if (fastStepDelayMs < SPEED_MIN_MS) fastStepDelayMs = SPEED_MIN_MS;
  if (fastStepDelayMs > SPEED_MAX_MS) fastStepDelayMs = SPEED_MAX_MS;
  if (slowStepDelayMs < SPEED_MIN_MS) slowStepDelayMs = SPEED_MIN_MS;
  if (slowStepDelayMs > SPEED_MAX_MS) slowStepDelayMs = SPEED_MAX_MS;
}
...
// Simplified LED update functions - honest about blocking
void updateLEDsIfNeeded() {
  unsigned long now = millis();
  if (ledUpdatePending && (now - lastLedUpdateMs >= ledUpdateIntervalMs)) {
    strip.show();  // This WILL block for 60-120ms, but only every 50ms max
    ledUpdatePending = false;
    lastLedUpdateMs = now;
  }
}

void markLEDsForUpdate() {
  ledUpdatePending = true;
}

// Update LED interval and animation speed based on IR activity
void updateLEDIntervalBasedOnIR() {
  unsigned long now = millis();
  
  // Check if IR is currently active (within timeout period)
  bool irActive = (now - lastIRSignalMs) < irActivityTimeoutMs;
  
  if (irActive) {
    // IR is active - use slower LED updates and normal animation speed
    ledUpdateIntervalMs = maxLedIntervalMs; // 400ms = 2.5 FPS
    stepDelayMs = slowStepDelayMs;          // 30ms = normal speed
  } else {
    // IR is idle - use faster LED updates and smoother animations
    ledUpdateIntervalMs = minLedIntervalMs; // 200ms = 5 FPS
    stepDelayMs = fastStepDelayMs;          // 15ms = faster animations
  }
}
...
void handleIR() {
  ...
  // Track IR activity for dynamic LED timing
  ...
  Serial.print(F(" | LED interval: ")); Serial.print(ledUpdateIntervalMs); Serial.println(F("ms (IR active - slow)"));
  ...
  // RYCHLOST: LEFT / RIGHT (adjusts both fast and slow speeds)
  else if (signal == LEFTBTN) {
    if (fastStepDelayMs + SPEED_STEP_MS <= SPEED_MAX_MS) fastStepDelayMs += SPEED_STEP_MS;
    if (slowStepDelayMs + SPEED_STEP_MS <= SPEED_MAX_MS) slowStepDelayMs += SPEED_STEP_MS;
    clampSpeed();
    Serial.print(F("<< SPEED slower: fast=")); Serial.print(fastStepDelayMs); 
    Serial.print(F("ms, slow=")); Serial.print(slowStepDelayMs); Serial.println(F("ms"));
  } else if (signal == RIGHTBTN) {
    if (fastStepDelayMs >= SPEED_STEP_MS) fastStepDelayMs -= SPEED_STEP_MS;
    if (slowStepDelayMs >= SPEED_STEP_MS) slowStepDelayMs -= SPEED_STEP_MS;
    clampSpeed();
    Serial.print(F(">> SPEED faster: fast=")); Serial.print(fastStepDelayMs); 
    Serial.print(F("ms, slow=")); Serial.print(slowStepDelayMs); Serial.println(F("ms"));
  }
...
void loop() {
  // Handle IR input first (highest priority)
  handleIR();
  
  // Update LED interval based on IR activity
  updateLEDIntervalBasedOnIR();
  
  // Update LEDs if needed (dynamic throttling based on IR activity)
  updateLEDsIfNeeded();
  ...
  if (now - lastStepMs >= stepDelayMs) {
    ...
    // Run animation (simplified - no frame skipping)
    switch (currentShow) {
