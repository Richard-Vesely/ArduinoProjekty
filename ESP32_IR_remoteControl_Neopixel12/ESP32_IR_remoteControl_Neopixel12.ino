/******************************************************
ZAPOJENÍ (ESP32 + NeoPixel 12 + IR přijímač)
-------------------------------------------------------
NeoPixel 12 kruh (WS2812/WS2812B/NeoPixel):
- DIN (data)  -> GPIO 5 přes odpor ~330 Ω
- 5V          -> 5V (nebo externích 5V; GND společná!)
- GND         -> GND
(Doporučeno: kondenzátor 470–1000 µF mezi 5V a GND u LED.
Pro delší kabely doporučen level shifter 3,3V→5V pro DATA.)

IR přijímač (VS1838B/TSOP38238 apod.):
- OUT         -> GPIO 27 (IR_RECEIVE_PIN)
- VCC         -> 3,3V nebo 5V dle modulu
- GND         -> GND

POZNÁMKY:
- Pokud napájíš LED externím zdrojem, PROPOJ jeho GND s GND ESP32.
- WS2812: napájení 5V; WS2815: napájení 12V a data 5V (3,3V od ESP32 obvykle fungují,
  ale pro jistotu použij level shifter). Vždy společná zem!

OVLÁDÁNÍ IR OVLADAČEM
- Číslice 1–9: volba show (9 efektů).
- LEFT / RIGHT: zpomalit / zrychlit animaci.
- UP / DOWN: snížit / zvýšit jas.
- HASH (#): zapnout / vypnout LED.
...
5) Co se stane, když není společná GND mezi ESP32, IR a LED? Jak se to projeví?
******************************************************/

#include <Adafruit_NeoPixel.h>
#include <IRremote.hpp>    // IRremote v3+
#include <MyIRcodes.h>     // tvoje header-only knihovna s pojmenovanými kódy
...
bool ledsOn = true;   // přepínání ON/OFF pomocí HASH
...
void handleIR() {
  ...
  // Shows
  if (signal == ONE)    { currentShow = 1; animStep = 0; Serial.println(F(">> SHOW 1")); }
  else if (signal == TWO)   { currentShow = 2; animStep = 0; Serial.println(F(">> SHOW 2")); }
  else if (signal == THREE) { currentShow = 3; animStep = 0; Serial.println(F(">> SHOW 3")); }
  else if (signal == FOUR)  { currentShow = 4; animStep = 0; memset(cometTrail,0,sizeof(cometTrail)); cometPos=0; Serial.println(F(">> SHOW 4")); }
  else if (signal == FIVE)  { currentShow = 5; animStep = 0; wipeIndex=0; wipeColorIndex=0; Serial.println(F(">> SHOW 5")); }
  else if (signal == SIX)   { currentShow = 6; animStep = 0; memset(twinkleVal,0,sizeof(twinkleVal)); Serial.println(F(">> SHOW 6")); }
  else if (signal == SEVEN) { currentShow = 7; animStep = 0; scanPos=0; scanDir=1; Serial.println(F(">> SHOW 7")); }
  else if (signal == EIGHT) { currentShow = 8; animStep = 0; for (uint8_t i=0;i<MAX_DROPS;i++){drops[i].life=0;} Serial.println(F(">> SHOW 8")); }
  else if (signal == NINE)  { currentShow = 9; animStep = 0; palIdx=0; Serial.println(F(">> SHOW 9")); }

  // Speed
  else if (signal == LEFTBTN) {
    if (stepDelayMs + SPEED_STEP_MS <= SPEED_MAX_MS) stepDelayMs += SPEED_STEP_MS;
    clampSpeed();
    Serial.print(F("<< SPEED slower: ")); Serial.println(stepDelayMs);
  } else if (signal == RIGHTBTN) {
    if (stepDelayMs >= SPEED_STEP_MS) stepDelayMs -= SPEED_STEP_MS;
    clampSpeed();
    Serial.print(F(">> SPEED faster: ")); Serial.println(stepDelayMs);
  }
...
void setup() {
  Serial.begin(115200);
  delay(50);
  Serial.println(F("ESP32: IR + NeoPixel 12 — 9 shows + HASH=ON/OFF"));
